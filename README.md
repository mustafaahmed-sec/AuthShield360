...
2. Configure Gmail SMTP for email OTP, then configure Firebase Phone Auth if the team enables live SMS verification.
...
The login supports email OTP through Gmail SMTP and SMS verification through Firebase Authentication. Email codes expire after `AUTHSHIELD_EMAIL_OTP_TTL_SECONDS` (60 seconds by default), and the page shows a live minutes-and-seconds countdown plus the resend wait. The branded HTML email greets the account holder by name, displays the code clearly, and includes a plain-text version for email clients that do not display HTML. Repeated requests while a code is active reuse the same code instead of sending duplicate messages; an explicit replacement after 30 seconds invalidates the previous code. Migration `0006_email_otp_challenge` stores one hashed active challenge for each account and purpose. The sign-in page includes a password recovery flow that sends a one-minute email code, uses the same single-active-code behavior, and checks the new password against the portal's password rules. Password recovery responses do not reveal whether an email belongs to an approved account. Apply hosted database migrations before enabling password recovery. Firebase controls its SMS code format and delivery; the portal limits the pending sign-in session to `AUTHSHIELD_SMS_OTP_TTL_SECONDS` (300 seconds by default), code attempts, and resends. The browser uses Firebase's reCAPTCHA-protected phone flow, and Django verifies the signed Firebase ID token and checks that it proves fresh verification of the phone number saved on the account. `AUTHSHIELD_EMAIL_STEP_UP=false` (the default) lets users choose SMS or email when Firebase is configured; otherwise only email is offered. Set `AUTHSHIELD_EMAIL_STEP_UP=true` to require SMS followed by a second Gmail email code. Set `AUTHSHIELD_OTP_ENABLED=true` only after configuring and enabling the chosen authentication method.
